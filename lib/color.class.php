...
class Color_class {
	...
	protected static $table_name			= 'color'; // Data base table name
	...
	public static function find_by_id($id=0){
		global $database;
		$result_array = self::find_by_sql("SELECT * FROM " .self::$table_name. " WHERE color_id={$id}");
	...
	public function save(){
		return isset($this->color_id) ? $this->update() : $this->create();
	...
	public function create() {
		...
		if($database->query($sql)){
			$this->color_id = $database->insert_id();
	...
	public function update() {
		...
		$sql .= " WHERE color_id =". $database->escape_value($this->color_id);
