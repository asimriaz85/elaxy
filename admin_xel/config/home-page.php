...
if(isset($_REQUEST['submit'])){
	...
	if(!empty($ad_page_name)&&!empty($location)&&!empty($hide_show)){
		...
		$insert="INSERT INTO adsens_ad(ad_page_name,location,images,status,ad_code) values('$ad_page_name','$location',
'$client_image','$hide_show','$ad_code')" or die (mysql_error());
...
$name5="adsens_1_".$adsens_id.".jpg";
		...
		if($filetype3=="jpg" || "JPG" || "JPEG" || "jpeg" || "gif" || "png")
		{
			...
			if (move_uploaded_file($tmp_name5, "../media/upl_gallery/$name5"))
			{
				 ...
				 mysql_query("Update adsens_ad Set images='$name5' where id='$adsens_id'")or die(mysql_error());
				$msg="yes";
			}else{
			//if upload fails
			echo $msg="no";
