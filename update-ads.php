...
include("inc/validation.php");
	 ...
	 if(isset($_REQUEST['update'])){
		 ...
		 $up_date=$_REQUEST['date'];
		$up_link_site=$_REQUEST['link_site'];
		 $up_link_website=$_REQUEST['link_website'];
		 
		
		 
		 $up_youtube_link=$_REQUEST['youtube_link'];
		 
		 if(!empty($up_date)){
			 
			  $update_date="UPDATE postcode_location SET date='".$up_date."' WHERE id='".$postcode_id."'";
			$run_date=mysql_query($update_date);

		 }
		 
		 
		 if(!empty($up_title) && !empty($up_description)){
   ...
   $select_price= $pieces[1]; // piece2
		 ...
		 $selecet_your_web_link2="SELECT postcode_loaction_id,link_site,price FROM your_web_link WHERE postcode_loaction_id='".$postcode_id."'";
	$run_select_your_link2=mysql_query($selecet_your_web_link2);
	$your_link_num=mysql_num_rows($run_select_your_link2);
	if($your_link_num > 0){
	
		 $update_your_link="UPDATE your_web_link SET link_site='".$up_link_site."',price='".$up_link_website."' WHERE postcode_loaction_id='".$postcode_id;
		 $run_update_your_link=mysql_query($update_your_link);
	
	}
	if($your_link_num ==0){
		
		$insert="INSERT INTO your_web_link(postcode_loaction_id,link_site,price)VALUES('".$postcode_id."','".$up_link_site."','".$up_link_website."')";
		$run=mysql_query($insert);
	}
		 
		 
		 $selecet_youtub_link1="SELECT postcode_loaction_id,youtube_link FROM youtube_link WHERE postcode_loaction_id='".$postcode_id."'";
	$run_select_youtub_link1=mysql_query($select_youtub_link1);
	$youtube_link_num=mysql_num_rows($run_select_youtub_link1);
	
	if($youtube_link_num >0){
	
		 
		 $update_youtube_link="UPDATE youtube_link SET youtube_link='".$up_youtube_link."' WHERE postcode_loaction_id='".$postcode_id."'";
		 $run_update_youtube_link=mysql_query($update_youtube_link);
		 
	}
	
	if($youtube_link_num==0){
		
	$insert_youtube="INSERT INTO youtube_link(postcode_loaction_id,youtube_link)VALUES('".$postcode_id."','".$youtube_link."')";
	$run_insert_youtube=mysql_query($insert_youtube);
	}
		 
		if($run_date || $run_updte || $run_contact){
			?>
<script type="text/javascript">window.location.href = 'manage_ads.php';</script>
		<?php	//header("location:manage_ads.php");
		}
	 }
	...
	$selecet_your_web_link="SELECT postcode_loaction_id,link_site,price FROM your_web_link WHERE postcode_loaction_id='".$postcode_id."'";
	$run_select_your_link=mysql_query($select_your_web_link);
	$fetch_your_link=mysql_fetch_array($run_select_your_link);
	$post_id_your_link=$fetch_your_link['postcode_loaction_id'];
	$link_site = $fetch_your_link['link_site'];
	$price=$fetch_your_link['price'];
	
	
	$selecet_youtub_link="SELECT postcode_loaction_id,youtube_link FROM youtube_link WHERE postcode_loaction_id='".$postcode_id."'";
	$run_select_youtub_link=mysql_query($selecet_youtub_link);
	$fetch_youtub_link=mysql_fetch_array($run_select_youtub_link);
	
	$youtube_link=$fetch_youtub_link['youtube_link'];
...
<div id="contant_area" style="position:relative;">
...
<?php if(isset($msg)){?><h1 style="color:#F00;"><?php echo $msg; ?></h1><?php } ?>
        <h1>Post your Ad</h1>
             ...
             <div class="Right">
              ...
              <div class="Right">
             	...
             	<div class="catHolder">
                	...
                	<label><input type="checkbox" class="cb" onclick="doit()" name="link_website" rel="7.50" id="link_website" value="7.50" <?php if($post_id_your_link==$postcode_id){ ?> checked<?php } ?> /> Link to your website:</label>
                	
                    <input type="text" name="link_site" value="<?php echo $fetch_web_link['link_site']; ?>" />                                 
                </div>                                            
             </div>   
             
             <div class="Right">

             	<h4>Add Your Youtube Link</h4>
             	<div class="catHolder">
                	
                	<label>Youtube Link:</label>
                	<input type="text" name="youtube_link" value="<?php echo $youtube_link; ?>" />                                 
                </div>                                            
             </div>
...
$pos = strpos($mystring, $findme); if ($pos !== false) {?> checked="checked"<?php }?> /> Spotlight</label>
             ...
             <!--<div class="Right">
             <h4>On Click check box update the ad date</h4>
             	<div class="catHolder">
                	<label><b>Update Ad Published Date:</b></label>
                	    <input type="checkbox" name="date" value="<?php //echo $cdate; ?>">           
             ...
             <div class="Right">
             	<div class="catHolder">
                	<label style="width:65%; line-height:42px;">By clicking 'Update My Ad', you agree to Elaxy's posting rules and terms of use.</label>
                	<input id="category-select_submit-category" name="update" class="button pull-right" value="Update My Ad" data-parent-component="#post-ad-category-select" type="submit">                 
                </div>                                            
             </div>                                                                                             
