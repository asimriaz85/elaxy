...
postcode_location.status ='1'  ";
	...
	if($_GET['searchby'] == 'town'){
		...
		$byloc = " AND postcode_location.town = '" . mysql_real_escape_string($_GET['town']) ."' ";

	} else if($_GET['searchby'] == 'county'){

		

		$byloc = " AND postcode_location.town IN(select town_name from town where county_id IN(select id from county where county_name = '" . mysql_real_escape_string($_GET['county']) . "'))";	

		

	} else if($_GET['searchby'] == 'region'){
	...
	$sql_search_view = $sql_search_view . $byloc;
...
$sql_search_view = $sql_search_view . $bykeyword . $by_category . $by_mils. " order by postcode_location.date desc";
...
<!--
	...
	<div style="width:600px; height:200px;">
    	...
    	<?php
			...
			//print_r($get_postcode);

//			echo $sql_search_view;

			//foreach($get_postcode as $p){ echo '<br>' .$p; }
...
<div class="list-image"><a href="http://elaxy.co.uk/ad/<?=$url?>"><?php if(!empty($image_name)) { ?><img src="/uploads/<?php echo $image_name; ?>" width="121" height="84" alt="" /><?php } else { ?> <img src="/images/noimage_thumbnail.png" width="121" height="84" alt="" />  <?php } ?></a></div>
...
<div class="list-pricing">
 ...
 $found=get_top($id2srch); if($found==true){  } else { ?>
