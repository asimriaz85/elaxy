	...
	$fetch_pname=mysql_fetch_array($run_select_pname);
	 ...
	 // $select_views="SELECT post_id,user_id FROM views WHERE post_id='".$newpostid."' AND user_id='".$user_id."'";
	  ...
	  //$view_user_id=$fetch_views['user_id'];
		  ...
		  $insert_views="INSERT INTO views set post_id='".$newpostid."'";
...
<script type="text/javascript">stLight.options({publisher: "d450a8b9-c1d2-4754-98cb-65c62930d975", doNotHash: false, doNotCopy: false, hashAddressBar: false});</script>
...
<li><a href="/config/getfavourite.php?post_id=<?php echo $newpostid; ?>&f_id=<?php echo $f_id; ?>&feature=<?php echo $feature; ?>&uid=<?php echo $user_id; ?>&sub_cat_id=<?php echo $sub_cat_id; ?>&url=<?=$_GET['url']?>"><img src="/img/folder_yellow_favourite.png" width="24" height="24" alt="star"  />Favourite</a></li>
...
<li style="float:right;"><span class='st_sharethis_large' displayText='ShareThis'></span>
...
if($sub_cat_id!='125' && $sub_cat_id!='518' && $sub_cat_id!='519' && $sub_cat_id!=520 && $sub_cat_id!='521' && $sub_cat_id!='522' && $sub_cat_id!='523' && $sub_cat_id!='529' && $sub_cat_id!='602'){
...
if(strstr($ex[1],"Flats & Houses")=="Flats & Houses"){



$select_property_holiday_rent="SELECT * FROM property_holiday_rent WHERE postcode_loaction_id='".$newpostid."'";$run_property_holiday_rent=mysql_query($select_property_holiday_rent);
...
<?php if(!empty($rent_period)){ ?>
<li>
<h5>Rent Period</h5>
<p><?=$rent_period?></p>
</li>
<?php }  if(!empty($acting_agent)){?>
<li>
<h5>Seller Type</h5>
<p><?=$acting_agent?></p>
</li>
...
<?php if( $countutube > 0){ ?>
            ...
            <a href="http://<?=$rsutube['youtube_link']?>" target="_blank">www.<?=$rsutube['youtube_link']?></a>
            ...
            <a href="http://<?=$rsweb['link_site']?>" target="_blank">www.<?=$rsweb['link_site']?></a>
...
<div class="slider">
 ...
 <ul class="nav nav-tabs">
    ...
    <li class="active"><a href="#map" data-toggle="tab" style="font-weight:bold;">MAP</a></li>

  <li ><a href="#images" data-toggle="tab" style="font-weight:bold;">REPLY TO THIS AD</a></li>
...
<div class="tab-content" style="border:1px solid #dddddd; border-top:none;">
  ...
  <div class="tab-pane " id="images"><div class="thumbs clearfix">
...
<form class="pure-form pure-form-stacked" action="/submit.php?post_id=<?php echo $newpostid; ?>&user_id=<?php echo $user_id; ?>&f_name=<?php echo $f_name; ?>" id="form" style="margin-left:10px;" method="post">

<input type="hidden" name="post_id" value="<?=$newpostid?>">
...
height: 140px; resize:none;"></textarea>
  ...
  <div class="tab-pane active" id="map"><div id="map_canvas" style="width: 425px; float: left; clear: both; height: 200px; border: 2px solid #F0EEEC; position: relative; background-color: #E5E3DF;"></div></div>
...
<h2> <span style="float:right;"><img src="/images/clear.png" width="16" height="16"><a href="/clear-history.php?emptycart=1&return_url=<?=$current_url?>">Clear history</a></span></h2>
...
<?php
  ...
  //prepare array for the session variable
		...
		$new_product = array(array('name'=>$ad_title,'code'=>$newpostid,'price'=>$price,'image'=>$ad_image1,'url'=>$_GET['url']));
...
<div><a href="<?=$cart_itm["url"]?>"><?php if(!empty($cart_itm["image"])) { ?><img src="/thumb.php?src=/uploads/<?=$cart_itm["image"]?>&w=110&h=84" width="110" height="84" alt="" /><?php } else { ?> <img src="/images/noimage_thumbnail.png" width="121" height="84" alt="" />  <?php } ?>
