...
<ul id="list" class="leftside_ul">
    ...
    <li style="color:#000;"><a href="http://elaxy.co.uk/cat/<?=$purl?>" style="color:#FFF;"><?php echo $pname; ?>&nbsp;<?php echo "($total_p_ad)"; ?></a>
    <ul>
...
foreach($subcat as $subcatlist){
	...
	//Total CAt1 Ad//
$total_subsub_add = "SELECT id, COUNT(sub_sub_cat) FROM postcode_location WHERE sub_sub_cat='".$subcatlist->id."' AND status='1'";
...
?>
            <li style="font-size:12px;"><a style="color:#FFF;" href="http://elaxy.co.uk/cat/<?=$subcatlist->cat_url;?>" ><?php echo $subcatlist->name; ?>&nbsp;<?php echo "($total_subsub_add)"; ?></a></li>
...
$subhidecat=Category_Class::find_by_sql("SELECT * FROM categories WHERE parent_off='".$catname->id."' ORDER BY name asc LIMIT 5,300");


	foreach($subhidecat as $subhid){
	//Total CAt1 Ad//
$total_subsub_add = "SELECT id, COUNT(sub_sub_cat) FROM postcode_location WHERE sub_sub_cat='".$subhid->id."' AND status='1'"; 
...
?>
            <li id="<?php echo $subhid->id; ?>" style="font-size:12px; display:none;"><a  style="color:#FFF;" href="http://elaxy.co.uk/cat/<?=$subhid->cat_url;?>"><?php echo $subhid->name; ?>&nbsp;<?php echo "($total_subsub_add)"; ?></a></li>
