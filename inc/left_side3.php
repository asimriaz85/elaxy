...
<?php
$select_parent_category="SELECT id,name,cat_url FROM categories WHERE id='".$parent_categories."'";
...
$select_child="SELECT id,name,cat_url FROM categories WHERE parent_off='".$parent_id."' order by name asc";
...
$sub_cat_url=$fetch_child['cat_url'];
...
?>
        ...
        <?php 
$select_cat="SELECT * FROM categories WHERE parent_off='".$child_id."' order by name asc LIMIT 0,5";
...
?>
            ...
            <li style="font-size:12px;"><a <?php if($cat_id==$sub_cat_id){?>  class="leftside_selected"  href="#"<?php } if($cat_id!=$sub_cat_id){ ?>  style="color:#FFF;" href="http://elaxy.co.uk/cat/<?=$sub_cat_url?>" <?php } ?>><?php echo $cat_name; ?>&nbsp;<?php echo "($total_subsub_add)"; ?></a></li>
            <?php } ?>
            
           <li id='more' style="cursor:pointer;">View more</li>
                   <?php 
$select_cat="SELECT * FROM categories WHERE parent_off='".$child_id."' order by name asc LIMIT 5,300";
$run_select_cat=mysql_query($select_cat);
while($fetch_select_cat=mysql_fetch_array($run_select_cat)){
...
?>
            ...
            <li id="<?php echo $cat_id; ?>" style=" font-size:12px; display:none;"><a <?php if($cat_id==$sub_cat_id){?>  class="leftside_selected"  href="#"<?php } if($cat_id!=$sub_cat_id){ ?>  style="color:#FFF;" href="http://elaxy.co.uk/cat/<?=$sub_cat_url?>"<?php } ?>><?php echo $cat_name; ?>&nbsp;<?php echo "($total_subsub_add)"; ?></a></li>
            <?php } ?>
